...
def _parse_date_string_from_eds(d):
	...
	if isinstance(d, datetime):
		...
		try:
			...
			if e.args != "day is out of range for month":						# Error message generated by strptime when a month doesn't have that many days - OM addition, changed .message to .args
...
class _Connection:
	...
	def __init__(self, user_id, password):
		...
		self.auth_token						= ""
		self.auth_timeout					= "" 	#OM Addition - resolving W0201 Pylint error
		self.auth_timeout_time				= ""	#OM Addition - resolving W0201 Pylint error
		self.active							= False
	...
	def request(self, method, data, session_token=None, attempt=0):
		...
		r									= post(full_url, data=data_json, headers=headers, timeout=None) # OM addition adding timeout value set to default None to resolve W3101 Pylint error
...
class ConnectionPool(Borg):
	...
	def __init__(self):
		...
		self.new_user_id					= ""	#OM Addition - resolving W0201 Pylint error
		self.new_password					= ""	#OM Addition - resolving W0201 Pylint error
...
#			self.end()
	...
	def _search(self, search_data=None, expect_page=None):
		...
		if search_data is None:
			search_data={} # OM addition resolving W0102 Pylint error
		...
		if not expect_page or expect_page == self.current_page:
			...
			logging.warning("Session._search: Expected page %s but got page %s; Results object will be empty!", type(e), e (expect_page, self.current_page))	# OM Addition resolving W1201 Pylint error
	...
	def search(self, query, mode=None, sort="relevance", view="brief", rpp=20, page=1, highlight="y", suggest="n", iqv="n", expanders=None, limiters=None):
		...
		if expanders is None:
			expanders = []	# OM addition resolving W0102 Pylint error

		if limiters is None:
			limiters = []	# OM addition resolving W0102 Pylint error
...
class Results:
	...
	def __init__(self):
		...
		self.rpp							= []							# OM addition resolving W0201 Pylint error				
		self.search_criteria				= []							# OM Addition resolving W0201 Pylint error
...
class Record:
	...
	def __init__(self):
		...
		self.simple_title					= ""
		self.simple_author					= ""
		self.simple_subject					= ""	#OM Addition resolving Pylint W0201 error
