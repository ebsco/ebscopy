...
#def _parse_date_string_to_string(d):
	#"""
...
#
	#This function intentionally returns the original string on success so that it can be called from _parse_date_string_from_eds().
	#
	#:param str d: string containing the 8-digit date
	#:rtype: str
	#"""
	#try:
	...
	#except ValueError as e:
			...
			#raise																# Something else went wrong, so let it bubble up
				...
				#return _parse_date_string_to_string(str(int(d)-1))				# I feel equally horrible and awesome for this use of recursion
			#else:
				#raise
	##return d
## End of [_parse_date_string_to_string] function
...
def _parse_date_string_from_eds(d):
	...
	#d						= _parse_date_string_to_string(d)
...
def _change_eds_date_limiter_to_api(l):
	...
	start_date								= _parse_date_string_from_eds(start_str)
	end_date								= _parse_date_string_from_eds(end_str)
